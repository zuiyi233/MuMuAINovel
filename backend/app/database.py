...
# 模型导入由 Alembic env / 应用启动阶段统一处理，避免在 database.py 导入期触发循环依赖。
# 此模块仅负责 Base、engine 与 session 管理。
...
async def get_db(request: Request):
    ...
    try:
        ...
        logger.warning(
            f"⚠️ GeneratorExit [User:{user_id}][ID:{session_id}] - SSE连接断开（总计:{_session_stats['generator_exits']}次）"
        )
        try:
            if session.in_transaction():
                await session.rollback()
            ...
            logger.error(
                f"❌ GeneratorExit回滚失败 [User:{user_id}][ID:{session_id}]: {str(rollback_error)}"
            )
    except Exception as e:
        _session_stats["errors"] += 1
        ...
        try:
            ...
            logger.error(
                f"❌ 异常回滚失败 [User:{user_id}][ID:{session_id}]: {str(rollback_error)}"
            )
        raise
    finally:
        try:
            ...
            if _session_stats["active"] > settings.database_session_leak_threshold:
                logger.error(
                    f"🚨 严重告警：活跃会话数 {_session_stats['active']} 超过泄漏阈值 {settings.database_session_leak_threshold}！"
                )
                ...
                logger.error(
                    f"🚨 活跃会话数异常: {_session_stats['active']}，统计可能不准确！"
                )

        except Exception as e:
            _session_stats["errors"] += 1
            logger.error(
                f"❌ 关闭会话时出错 [User:{user_id}][ID:{session_id}]: {str(e)}",
                exc_info=True,
            )
            try:
                await session.close()
...
async def get_database_stats():
    ...
    stats = {
        ...
        "config": {
            ...
            "pool_size": settings.database_pool_size,
            "max_overflow": settings.database_max_overflow,
            "total_connections": settings.database_pool_size
...
async def check_database_health(user_id: str = None) -> dict:
    ...
    Returns:
        dict: 健康检查结果
    """
    result = {"healthy": True, "checks": {}, "timestamp": datetime.now().isoformat()}
