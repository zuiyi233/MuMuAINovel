...
async def get_user_ai_service(
    ...
    if not settings:
        ...
        settings = Settings(user_id=user.user_id, **env_defaults)
        db.add(settings)
        await db.commit()
    ...
    if mcp_plugins:
        ...
        logger.info(
            f"用户 {user.user_id} 有 {len(mcp_plugins)} 个MCP插件，{enabled_count} 个启用，{enable_mcp} 决定使用MCP"
        )
    else:
        ...
        if isinstance(project_id, str) and project_id:
            ...
            project_result = await db.execute(
                select(Project).where(
                    Project.id == project_id, Project.user_id == user.user_id
                )
            )
            ...
            if project and project.active_skill_key:
                ...
                ps = await db.execute(
                    select(SkillSpec).where(
                        SkillSpec.skill_key == project.active_skill_key
                    )
                )
    ...
    try:
        ...
        if isinstance(active_skill_key, str) and active_skill_key:
            ...
            skill_result = await db.execute(
                select(SkillSpec).where(SkillSpec.skill_key == active_skill_key)
            )
    ...
    local_skill_prompt = getattr(request_state, "local_skill_prompt", None)
    local_skill_id = getattr(request_state, "local_skill_id", None)
    if isinstance(local_skill_prompt, str) and local_skill_prompt.strip():
        local_id_line = f"\nID: {local_skill_id}" if local_skill_id else ""
        local_block = (
            f"\n\n# 本地技能（临时）{local_id_line}\n{local_skill_prompt.strip()}\n"
        )
        base_prompt = (final_system_prompt or "").strip()
        final_system_prompt = (
            (base_prompt + local_block) if base_prompt else local_block.strip()
        )
...
async def get_settings(
    user: User = Depends(require_login), db: AsyncSession = Depends(get_db)
):
    """
    获取当前用户的设置
...
async def save_settings(
    ...
    if settings:
        ...
        try:
            ...
            if active_preset:
                ...
                config_changed = (
                    ...
                    != settings_dict.get("api_provider", settings.api_provider)
                    or preset_config.get("api_key")
                    != settings_dict.get("api_key", settings.api_key)
                    or preset_config.get("api_base_url")
                    != settings_dict.get("api_base_url", settings.api_base_url)
                    or preset_config.get("llm_model")
                    != settings_dict.get("llm_model", settings.llm_model)
                    or preset_config.get("temperature")
                    != settings_dict.get("temperature", settings.temperature)
                    or preset_config.get("max_tokens")
                    != settings_dict.get("max_tokens", settings.max_tokens)
                )
                ...
                if config_changed:
                    ...
                    logger.info(
                        f"用户 {user.user_id} 手动修改配置，已取消预设 {active_preset.get('name')} 的激活状态"
                    )
        ...
        # 创建新设置
        settings = Settings(user_id=user.user_id, **settings_dict)
        db.add(settings)
        await db.commit()
        await db.refresh(settings)
        apply_vector_embedding_from_preferences(settings)
...
async def get_available_models(
    api_key: str, api_base_url: str, provider: str = "openai"
):
    """
    从配置的 API 获取可用的模型列表
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            if provider == "openai" or provider == "azure" or provider == "custom":
                ...
                if not models:
                    raise HTTPException(
                        status_code=404, detail="未能从 API 获取到可用的模型列表"
                    )
                ...
                raise HTTPException(
                    status_code=400, detail=f"不支持的提供商: {provider}"
                )

    except httpx.HTTPStatusError as e:
        logger.error(
            f"获取模型列表失败 (HTTP {e.response.status_code}): {e.response.text}"
        )
        raise HTTPException(
            status_code=400,
...
async def check_function_calling_support(data: ApiTestRequest):
    ...
    try:
        ...
        result = {
            "success": True,
            "supported": supported,
            "message": "✅ 模型支持 Function Calling"
            if supported
            else "❌ 模型不支持 Function Calling",
...
async def test_api_connection(data: ApiTestRequest):
    ...
    try:
        ...
        if "blocked" in error_msg.lower():
            ...
            suggestions = ["API 请求频率超限", "建议：稍后重试", "建议：升级 API 套餐"]
        elif "insufficient" in error_msg.lower() or "quota" in error_msg.lower():
            suggestions = ["API 配额不足", "建议：检查账户余额", "建议：充值或升级套餐"]
        else:
            suggestions = [
                "请检查所有配置参数是否正确",
                "请确认网络连接正常",
...
async def get_presets(
    ...
    # 找到激活的预设
    active_preset_id = next((p["id"] for p in presets if p.get("is_active")), None)
...
async def delete_preset(
    ...
    if target_preset.get("is_active"):
        raise HTTPException(
            status_code=400, detail="无法删除激活中的预设，请先激活其他预设"
        )
...
async def create_preset_from_current(
    ...
    create_request = PresetCreateRequest(
        name=name, description=description, config=current_config
    )
